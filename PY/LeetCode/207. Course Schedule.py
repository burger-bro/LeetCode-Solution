...
class Solution:
    def canFinish0(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        length = numCourses
        graph = [[False]*length for _ in range(length)]

        set1 = set()
        set2 = set()
        for pre in prerequisites:
            set1.add((pre[1], pre[0]))
            set2.add((pre[0], pre[1]))
            ...
            for i in range(length):
                if graph[id][i]:
                    ...
                    if i in _visited:
                        ...
                        return False
                    _visited.add(i)
                    if not dfs(i, _visited):
                        ans = False
                    _visited.remove(i)
            return ans

        # print(source_list)
        # for source in (source_list if source_list else range(length)):
...
numCourses = 6
prerequisites = [[1,0], [5,0], [2,3]]
print(su.canFinish(numCourses, prerequisites))
